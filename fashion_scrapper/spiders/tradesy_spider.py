...
class TradesySpider(scrapy.Spider):
    ...
    def parse(self, response):
        for idx, category_section in enumerate(response.css('ul.trd-menu li')):
            ...
            category_column = category_section.css('div.trd-dropdown.collection ul')
    ...
    def parse_category_page(self, response):
        ...
        if len(subcategories) > 0:
            ...
            total_page_number_str = total_page_number_selector[-2].xpath('//text()')
            if total_page_number_selector[-1].xpath('@class') != 'page-next':
                total_page_number_str = total_page_number_selector[-1].xpath('//text()')
            total_page_number = int(total_page_number_str)
            products = response.css('a.item-image::attr(href)')
